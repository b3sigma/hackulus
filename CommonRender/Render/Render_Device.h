...
enum MapFlags {
  Map_Discard = 1, Map_Read = 2, // do not use
...
public:
  ...
  bool TestRay(const Vector3f& origin, const Vector3f& norm, float& len,
      Planef* ph = NULL) const;
};
...
public:
  Node()
      : Pos(Vector3f(0)), MatCurrent(1) {
  }
  virtual ~Node() {
  }

  enum NodeType {
    Node_NonDisplay, Node_Container, Node_Model
  };
...
struct Vertex {
  ...
  Vertex(const Vector3f& p, const Color& c, float u, float v, float u2,
      float v2, Vector3f n)
      : Pos(p), C(c), U(u), V(v), U2(u2), V2(v2), Norm(n) {
  }
...
public:
  ...
  UInt16 AddVertex(const Vector3f& v, const Color& c, float u_ = 0,
      float v_ = 0) {
  ...
  UInt16 AddVertex(float x, float y, float z, const Color& c, float u1,
      float v1, float u2, float v2, float nx, float ny, float nz) {
    return AddVertex(
        Vertex(Vector3f(x, y, z), c, u1, v1, u2, v2, Vector3f(nx, ny, nz)));
  }
  ...
  void AddSolidColorBox(float x1, float y1, float z1, float x2, float y2,
      float z2, Color c);

  static Model* CreateAxisFaceColorBox(float x1, float x2, Color xcolor,
      float y1, float y2, Color ycolor, float z1, float z2, Color zcolor);
  ...
  static Model* CreateCylinder(Color c, Vector3f origin, float height,
      float radius, int sides = 20);
  static Model* CreateCone(Color c, Vector3f origin, float height, float radius,
      int sides = 20);
  static Model* CreateSphere(Color c, Vector3f origin, float radius, int sides =
      20);
...
enum PostProcessType {
  PostProcess_None, PostProcess_Distortion
};

enum DisplayMode {
  Display_Window = 0, Display_Fullscreen = 1, Display_FakeFullscreen
};
...
public:
  enum CompareFunc {
    Compare_Always = 0, Compare_Less = 1, Compare_Greater = 2, Compare_Count
  };
  RenderDevice();
  ...
  virtual bool IsFullscreen() const {
    return Params.Fullscreen != Display_Window;
  }
  ...
  virtual Texture* CreateTexture(int format, int width, int height,
      const void* data, int mipcount = 1) {
    OVR_UNUSED5(format, width, height, data, mipcount);
    return NULL;
  }
  ...
  virtual void SetRenderTarget(Texture* color, Texture* depth = NULL,
      Texture* stencil = NULL) {
    OVR_UNUSED3(color, depth, stencil);
  }
  virtual void SetDepthMode(bool enable, bool write, CompareFunc func =
      Compare_Less) = 0;
  ...
  float MeasureText(const Font* font, const char* str, float size,
      float* strsize = NULL);
  virtual void RenderText(const Font* font, const char* str, float x, float y,
      float size, Color c);

  virtual void FillRect(float left, float top, float right, float bottom,
      Color c);
  virtual void FillGradientRect(float left, float top, float right,
      float bottom, Color col_top, Color col_btm);
  virtual void RenderImage(float left, float top, float right, float bottom,
      ShaderFill* image, unsigned char alpha = 255);
  ...
  void SetDistortionConfig(const DistortionConfig& config, StereoEye eye =
      StereoEye_Left) {
    Distortion = config;
